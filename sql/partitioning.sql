...
CREATE OR REPLACE FUNCTION _timeudb_functions.get_partition_for_key(val anyelement)
    RETURNS int
    AS '@MODULE_PATHNAME@', 'ts_get_partition_for_key' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.get_partition_hash(val anyelement)
    RETURNS int
