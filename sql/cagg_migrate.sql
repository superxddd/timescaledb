...
CREATE OR REPLACE FUNCTION _timeudb_functions.cagg_migrate_plan_exists (
...
$BODY$
    SELECT EXISTS (
        SELECT 1
        FROM _timeudb_catalog.continuous_agg_migrate_plan
...
CREATE OR REPLACE FUNCTION _timeudb_functions.cagg_migrate_pre_validation (
    _cagg_schema TEXT,
    _cagg_name TEXT,
    _cagg_name_new TEXT
)
RETURNS _timeudb_catalog.continuous_agg
LANGUAGE plpgsql AS
$BODY$
DECLARE
    _cagg_data _timeudb_catalog.continuous_agg;
BEGIN
    SELECT *
    INTO _cagg_data
    ...
    IF _timeudb_functions.cagg_migrate_plan_exists(_cagg_data.mat_hypertable_id) IS TRUE THEN
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_create_plan (
    _cagg_data _timeudb_catalog.continuous_agg,
    _cagg_name_new TEXT,
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1
        FROM _timeudb_catalog.continuous_agg_migrate_plan_step
    ...
    INSERT INTO
        _timeudb_catalog.continuous_agg_migrate_plan (mat_hypertable_id, user_view_definition)
    VALUES (
    ...
    IF _integer_interval IS NOT NULL THEN
        ...
        IF _bucket_column_type = 'bigint' THEN
            _watermark := COALESCE(_timeudb_functions.cagg_watermark(_cagg_data.mat_hypertable_id)::bigint, '-9223372036854775808'::bigint)::TEXT;
        ELSIF _bucket_column_type = 'integer' THEN
            _watermark := COALESCE(_timeudb_functions.cagg_watermark(_cagg_data.mat_hypertable_id)::integer, '-2147483648'::integer)::TEXT;
        ELSE
            _watermark := COALESCE(_timeudb_functions.cagg_watermark(_cagg_data.mat_hypertable_id)::smallint, '-32768'::smallint)::TEXT;
        END IF;
    ELSE
        ...
        IF _bucket_column_type = 'timestamp with time zone' THEN
            _watermark := COALESCE(_timeudb_functions.to_timestamp(_timeudb_functions.cagg_watermark(_cagg_data.mat_hypertable_id)), '-infinity'::timestamptz)::TEXT;
        ELSE
            _watermark := COALESCE(_timeudb_functions.to_timestamp_without_timezone(_timeudb_functions.cagg_watermark(_cagg_data.mat_hypertable_id)), '-infinity'::timestamp)::TEXT;
        END IF;
    END IF;
    ...
    INSERT INTO
        _timeudb_catalog.continuous_agg_migrate_plan_step (mat_hypertable_id, type, config)
    VALUES
    ...
    UPDATE _timeudb_catalog.continuous_agg_migrate_plan_step
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_create_new_cagg (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
$BODY$
DECLARE
...
BEGIN
    ...
    FROM timeudb_information.continuous_aggregates c
    WHERE c.view_schema = _cagg_data.user_view_schema
    AND c.view_name = _cagg_data.user_view_name;

    _view_def := format(
        'CREATE MATERIALIZED VIEW %s WITH (timeudb.continuous, timeudb.materialized_only=%L) AS %s WITH NO DATA;',
        _view_name,
    ...
    IF _compression_enabled IS TRUE THEN
        EXECUTE format('ALTER MATERIALIZED VIEW %s SET (timeudb.compress=true)', _view_name);
    END IF;
END;
$BODY$ SET search_path TO pg_catalog, pg_temp;

-- Disable policies
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_disable_policies (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
-- Enable policies
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_enable_policies (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
-- Copy policies
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_copy_policies (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
DECLARE
    ...
    _bgw_job _timeudb_config.bgw_job;
    _policy_id INTEGER;
    _config JSONB;
BEGIN
    ...
    FROM _timeudb_catalog.continuous_agg ca
    JOIN _timeudb_catalog.hypertable h ON (h.id = ca.mat_hypertable_id)
    ...
    FOR _bgw_job IN
        ...
        ORDER BY id
    LOOP
        _policy_id := nextval('_timeudb_config.bgw_job_id_seq');
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_refresh_new_cagg (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
BEGIN
    ...
    -- migrations.
    --
    -- Related issue: (https://github.com/timescale/timeudb/issues/4913)
    --
    RAISE WARNING
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_copy_data (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_override_cagg (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_drop_old_cagg (
    _cagg_data _timeudb_catalog.continuous_agg,
    _plan_step _timeudb_catalog.continuous_agg_migrate_plan_step
)
LANGUAGE plpgsql AS
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_execute_plan (
    _cagg_data _timeudb_catalog.continuous_agg
)
LANGUAGE plpgsql AS
$BODY$
...
BEGIN
    ...
    LOOP
        ...
        _call_stmt := pg_catalog.format('CALL _timeudb_functions.cagg_migrate_execute_%s($1, $2)', pg_catalog.lower(pg_catalog.replace(_plan_step.type, ' ', '_')));
...
BEGIN
    ...
    _cagg_data := _timeudb_functions.cagg_migrate_pre_validation(_cagg_schema, _cagg_name, _cagg_name_new);

    -- create new migration plan
    CALL _timeudb_functions.cagg_migrate_create_plan(_cagg_data, _cagg_name_new, override, drop_old);
    COMMIT;
    ...
    CALL _timeudb_functions.cagg_migrate_execute_plan(_cagg_data);

    -- Remove chunk metadata when marked as dropped
    PERFORM _timeudb_functions.remove_dropped_chunk_metadata(_cagg_data.raw_hypertable_id);

    -- finish the migration plan
    UPDATE _timeudb_catalog.continuous_agg_migrate_plan
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.cagg_migrate_to_time_bucket(cagg REGCLASS)
