...
GRANT SELECT ON _timeudb_catalog.dimension_partition TO PUBLIC;
...
ALTER TABLE _timeudb_catalog.chunk ADD COLUMN  osm_chunk boolean ;
UPDATE _timeudb_catalog.chunk SET osm_chunk = FALSE;
...
UPDATE
  _timeudb_config.bgw_job job
SET
  check_schema = '_timeudb_internal',
  check_name = 'policy_reorder_check'
WHERE proc_schema = '_timeudb_internal'
...
DROP VIEW IF EXISTS timeudb_information.jobs;-- cagg migration catalog relations
...
SELECT pg_catalog.pg_extension_config_dump('_timeudb_catalog.continuous_agg_migrate_plan_step_step_id_seq', '');

-- in tables.sql the same is done with GRANT SELECT ON ALL TABLES IN SCHEMA
GRANT SELECT ON _timeudb_catalog.continuous_agg_migrate_plan TO PUBLIC;
GRANT SELECT ON _timeudb_catalog.continuous_agg_migrate_plan_step TO PUBLIC;
GRANT SELECT ON _timeudb_catalog.continuous_agg_migrate_plan_step_step_id_seq TO PUBLIC;
