...
BEGIN
  ...
  JOIN pg_foreign_data_wrapper fdw ON srv.srvfdw = fdw.oid AND fdw.fdwname = 'timeudb_fdw';
  ...
  IF EXISTS(SELECT FROM _timeudb_catalog.metadata WHERE key = 'dist_uuid') THEN
    RAISE USING
      ERRCODE = 'feature_not_supported',
      MESSAGE = 'cannot upgrade because multi-node has been removed in 2.14.0',
...
DROP FUNCTION IF EXISTS _timeudb_functions.ping_data_node;
DROP FUNCTION IF EXISTS _timeudb_internal.ping_data_node;
DROP FUNCTION IF EXISTS _timeudb_functions.remote_txn_heal_data_node;
DROP FUNCTION IF EXISTS _timeudb_internal.remote_txn_heal_data_node;

DROP FUNCTION IF EXISTS _timeudb_functions.set_dist_id;
DROP FUNCTION IF EXISTS _timeudb_internal.set_dist_id;
DROP FUNCTION IF EXISTS _timeudb_functions.set_peer_dist_id;
DROP FUNCTION IF EXISTS _timeudb_internal.set_peer_dist_id;
DROP FUNCTION IF EXISTS _timeudb_functions.validate_as_data_node;
DROP FUNCTION IF EXISTS _timeudb_internal.validate_as_data_node;
DROP FUNCTION IF EXISTS _timeudb_functions.show_connection_cache;
DROP FUNCTION IF EXISTS _timeudb_internal.show_connection_cache;
...
CREATE TABLE _timeudb_catalog.compression_settings (
...
INSERT INTO _timeudb_catalog.compression_settings(relid, segmentby, orderby, orderby_desc, orderby_nullsfirst)
  SELECT
  ...
  FROM _timeudb_catalog.hypertable_compression hc
    INNER JOIN _timeudb_catalog.hypertable ht ON ht.id = hc.hypertable_id
  GROUP BY hypertable_id, ht.schema_name, ht.table_name;

INSERT INTO _timeudb_catalog.compression_settings
SELECT format('%I.%I',ch.schema_name,ch.table_name)::regclass,s.segmentby,s.orderby,s.orderby_desc,s.orderby_nullsfirst
FROM _timeudb_catalog.hypertable ht1
INNER JOIN _timeudb_catalog.hypertable ht2 ON ht2.id = ht1.compressed_hypertable_id
INNER JOIN _timeudb_catalog.compression_settings s ON s.relid = format('%I.%I',ht1.schema_name,ht1.table_name)::regclass
INNER JOIN _timeudb_catalog.chunk ch ON ch.hypertable_id = ht2.id ON CONFLICT DO NOTHING;

GRANT SELECT ON _timeudb_catalog.compression_settings TO PUBLIC;
SELECT pg_catalog.pg_extension_config_dump('_timeudb_catalog.compression_settings', '');

ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.hypertable_compression;
DROP VIEW IF EXISTS timeudb_information.compression_settings;
DROP TABLE _timeudb_catalog.hypertable_compression;

DROP FOREIGN DATA WRAPPER IF EXISTS timeudb_fdw;
DROP FUNCTION IF EXISTS @extschema@.timeudb_fdw_handler();
DROP FUNCTION IF EXISTS @extschema@.timeudb_fdw_validator(text[], oid);


DROP FUNCTION IF EXISTS _timeudb_functions.create_chunk_replica_table;
DROP FUNCTION IF EXISTS _timeudb_functions.chunk_drop_replica;
DROP PROCEDURE IF EXISTS _timeudb_functions.wait_subscription_sync;
DROP FUNCTION IF EXISTS _timeudb_functions.health;
DROP FUNCTION IF EXISTS _timeudb_functions.drop_stale_chunks;
...
DROP FUNCTION IF EXISTS _timeudb_internal.health;
DROP FUNCTION IF EXISTS _timeudb_internal.drop_stale_chunks;

ALTER TABLE _timeudb_catalog.remote_txn DROP CONSTRAINT remote_txn_remote_transaction_id_check;

DROP TYPE IF EXISTS @extschema@.rxid CASCADE;
DROP FUNCTION IF EXISTS _timeudb_functions.rxid_in;
DROP FUNCTION IF EXISTS _timeudb_functions.rxid_out;

DROP FUNCTION IF EXISTS _timeudb_functions.data_node_hypertable_info;
DROP FUNCTION IF EXISTS _timeudb_functions.data_node_chunk_info;
DROP FUNCTION IF EXISTS _timeudb_functions.data_node_compressed_chunk_stats;
DROP FUNCTION IF EXISTS _timeudb_functions.data_node_index_size;
DROP FUNCTION IF EXISTS _timeudb_internal.data_node_hypertable_info;
DROP FUNCTION IF EXISTS _timeudb_internal.data_node_chunk_info;
DROP FUNCTION IF EXISTS _timeudb_internal.data_node_compressed_chunk_stats;
DROP FUNCTION IF EXISTS _timeudb_internal.data_node_index_size;

DROP FUNCTION IF EXISTS timeudb_experimental.block_new_chunks;
DROP FUNCTION IF EXISTS timeudb_experimental.allow_new_chunks;
DROP FUNCTION IF EXISTS timeudb_experimental.subscription_exec;
...
DROP FUNCTION IF EXISTS _timeudb_functions.set_chunk_default_data_node;
DROP FUNCTION IF EXISTS _timeudb_internal.set_chunk_default_data_node;

DROP FUNCTION IF EXISTS _timeudb_functions.drop_dist_ht_invalidation_trigger;
DROP FUNCTION IF EXISTS _timeudb_internal.drop_dist_ht_invalidation_trigger;

-- remove multinode catalog tables
DROP VIEW IF EXISTS timeudb_information.chunks;
DROP VIEW IF EXISTS timeudb_information.data_nodes;
DROP VIEW IF EXISTS timeudb_information.hypertables;
DROP VIEW IF EXISTS timeudb_experimental.chunk_replication_status;

ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.remote_txn;
DROP TABLE _timeudb_catalog.remote_txn;
ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.hypertable_data_node;
DROP TABLE _timeudb_catalog.hypertable_data_node;
ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.chunk_data_node;
DROP TABLE _timeudb_catalog.chunk_data_node;
ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.chunk_copy_operation;
DROP TABLE _timeudb_catalog.chunk_copy_operation;
ALTER EXTENSION timeudb DROP SEQUENCE _timeudb_catalog.chunk_copy_operation_id_seq;
DROP SEQUENCE _timeudb_catalog.chunk_copy_operation_id_seq;
ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.dimension_partition;
DROP TABLE _timeudb_catalog.dimension_partition;

DROP FUNCTION IF EXISTS _timeudb_functions.hypertable_remote_size;
DROP FUNCTION IF EXISTS _timeudb_internal.hypertable_remote_size;
DROP FUNCTION IF EXISTS _timeudb_functions.chunks_remote_size;
DROP FUNCTION IF EXISTS _timeudb_internal.chunks_remote_size;
DROP FUNCTION IF EXISTS _timeudb_functions.indexes_remote_size;
DROP FUNCTION IF EXISTS _timeudb_internal.indexes_remote_size;
DROP FUNCTION IF EXISTS _timeudb_functions.compressed_chunk_remote_stats;
DROP FUNCTION IF EXISTS _timeudb_internal.compressed_chunk_remote_stats;

-- rebuild _timeudb_catalog.hypertable
...
DROP VIEW IF EXISTS timeudb_information.chunks;
DROP VIEW IF EXISTS timeudb_information.dimensions;
DROP VIEW IF EXISTS timeudb_information.compression_settings;
...
UPDATE _timeudb_catalog.hypertable h
SET status = 3
WHERE EXISTS (
  SELECT FROM _timeudb_catalog.chunk c WHERE c.osm_chunk AND c.hypertable_id = h.id
);
...
BEGIN
  ...
  FOR chunk, hypertable, ht_id IN
    SELECT
      ...
      ht.id
    FROM _timeudb_catalog.chunk ch
    INNER JOIN _timeudb_catalog.hypertable ht_uncomp
  ...
  LOOP
    ...
    DELETE FROM _timeudb_catalog.chunk_index WHERE hypertable_id = ht_id;
...
DROP FUNCTION IF EXISTS _timeudb_functions.hypertable_constraint_add_table_fk_constraint;

-- only define stub here, actual code will be filled in at end of update script
CREATE FUNCTION _timeudb_functions.constraint_clone(constraint_oid OID,target_oid REGCLASS) RETURNS VOID LANGUAGE PLPGSQL AS $$BEGIN END$$ SET search_path TO pg_catalog, pg_temp;

DROP FUNCTION IF EXISTS _timeudb_functions.chunks_in;
DROP FUNCTION IF EXISTS _timeudb_internal.chunks_in;

CREATE FUNCTION _timeudb_functions.metadata_insert_trigger() RETURNS TRIGGER LANGUAGE PLPGSQL
AS $$
BEGIN
  IF EXISTS (SELECT FROM _timeudb_catalog.metadata WHERE key = NEW.key) THEN
    UPDATE _timeudb_catalog.metadata SET value = NEW.value WHERE key = NEW.key;
    RETURN NULL;
  END IF;
  RETURN NEW;
END
...
CREATE TRIGGER metadata_insert_trigger BEFORE INSERT ON _timeudb_catalog.metadata FOR EACH ROW EXECUTE PROCEDURE _timeudb_functions.metadata_insert_trigger();

SELECT pg_catalog.pg_extension_config_dump('_timeudb_catalog.metadata', $$ WHERE key <> 'uuid' $$);
...
ALTER EXTENSION timeudb DROP TABLE _timeudb_cache.cache_inval_hypertable;
ALTER EXTENSION timeudb DROP TABLE _timeudb_cache.cache_inval_extension;
ALTER EXTENSION timeudb DROP TABLE _timeudb_cache.cache_inval_bgw_job;
ALTER EXTENSION timeudb DROP TABLE _timeudb_internal.job_errors;

-- Associate the above tables back to keep the dependencies safe
ALTER EXTENSION timeudb ADD TABLE _timeudb_cache.cache_inval_hypertable;
ALTER EXTENSION timeudb ADD TABLE _timeudb_cache.cache_inval_extension;
ALTER EXTENSION timeudb ADD TABLE _timeudb_cache.cache_inval_bgw_job;
ALTER EXTENSION timeudb ADD TABLE _timeudb_internal.job_errors;
...
CREATE FUNCTION _timeudb_functions.relation_approximate_size(relation REGCLASS)
