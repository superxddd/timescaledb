DROP FUNCTION IF EXISTS _timeudb_functions.remove_dropped_chunk_metadata(INTEGER);
...
WITH fixed_buckets AS (
    SELECT * FROM _timeudb_catalog.continuous_aggs_bucket_function WHERE bucket_fixed_width = true AND bucket_func::text LIKE '%time_bucket(interval%'
)
UPDATE _timeudb_catalog._tmp_continuous_agg cagg
    SET bucket_width = _timeudb_functions.interval_to_usec(fb.bucket_width::interval)
    FROM fixed_buckets fb
    WHERE cagg.mat_hypertable_id = fb.mat_hypertable_id;

-- Migrate CAggs with fixed bucket on integer back
WITH fixed_buckets AS (
    SELECT * FROM _timeudb_catalog.continuous_aggs_bucket_function WHERE bucket_fixed_width = true AND bucket_func::text NOT LIKE '%time_bucket(interval%'
)
UPDATE _timeudb_catalog._tmp_continuous_agg cagg
...
DELETE FROM _timeudb_catalog.continuous_aggs_bucket_function WHERE bucket_fixed_width = true;
...
--
UPDATE _timeudb_catalog.continuous_aggs_bucket_function SET bucket_origin = '' WHERE bucket_origin IS NULL;
UPDATE _timeudb_catalog.continuous_aggs_bucket_function SET bucket_timezone = '' WHERE bucket_timezone IS NULL;

CREATE TABLE _timeudb_catalog._tmp_continuous_aggs_bucket_function AS
    SELECT
      mat_hypertable_id,
      CASE WHEN bucket_func::text like 'timeudb_experimental%' THEN true ELSE false END,
...
CREATE TABLE _timeudb_catalog.continuous_aggs_bucket_function (
  mat_hypertable_id integer NOT NULL,
  -- The schema of the function. Equals TRUE for "timeudb_experimental", FALSE otherwise.
...
-- Convert _timeudb_catalog.continuous_aggs_bucket_function.origin back to Timestamp
UPDATE _timeudb_catalog.continuous_aggs_bucket_function
   SET origin = origin::timestamptz::timestamp::text
   WHERE length(origin) > 1;

-- only create stub
CREATE FUNCTION _timeudb_functions.get_chunk_relstats(relid REGCLASS)
...
CREATE FUNCTION _timeudb_functions.get_chunk_colstats(relid REGCLASS)
...
ALTER EXTENSION timeudb
  DROP VIEW timeudb_information.job_history;

DROP VIEW IF EXISTS timeudb_information.job_history;
...
INSERT INTO _timeudb_internal.job_errors (job_id, pid, start_time, finish_time, error_data)
SELECT
  job_id,
  pid,
  execution_start,
...
ALTER EXTENSION timeudb
    DROP TABLE _timeudb_internal.bgw_job_stat_history;

DROP TABLE IF EXISTS _timeudb_internal.bgw_job_stat_history;

REVOKE ALL ON _timeudb_internal.job_errors FROM PUBLIC;

DROP FUNCTION IF EXISTS _timeudb_internal.policy_job_stat_history_retention(job_id integer,config jsonb);
DROP FUNCTION IF EXISTS _timeudb_internal.policy_job_stat_history_retention_check(config jsonb);
DROP FUNCTION IF EXISTS _timeudb_functions.policy_job_stat_history_retention(job_id integer,config jsonb);
DROP FUNCTION IF EXISTS _timeudb_functions.policy_job_stat_history_retention_check(config jsonb);

CREATE OR REPLACE FUNCTION _timeudb_functions.policy_job_error_retention(job_id integer, config JSONB) RETURNS integer
LANGUAGE PLPGSQL AS
$BODY$
DECLARE
    drop_after INTERVAL;
...
BEGIN
    ...
    WITH deleted AS
        (DELETE
        FROM _timeudb_internal.job_errors
...
CREATE OR REPLACE FUNCTION _timeudb_functions.policy_job_error_retention_check(config JSONB) RETURNS VOID
LANGUAGE PLPGSQL AS
$BODY$
DECLARE
  drop_after interval;
...
UPDATE _timeudb_config.bgw_job SET scheduled = true WHERE id = 2;
DELETE FROM _timeudb_config.bgw_job WHERE id = 3;

DROP PROCEDURE IF EXISTS _timeudb_functions.cagg_migrate_to_time_bucket(cagg REGCLASS);
