CREATE FUNCTION _timeudb_internal.relation_size(relation REGCLASS)
...
DROP INDEX IF EXISTS _timeudb_catalog.chunk_constraint_chunk_id_dimension_slice_id_idx;
CREATE INDEX chunk_constraint_dimension_slice_id_idx ON _timeudb_catalog.chunk_constraint (dimension_slice_id);

-- Report the compressed chunks that have a wrong collation. See https://github.com/timescale/timeudb/pull/4236
DO $$
DECLARE
...
WITH chunk AS MATERIALIZED (
        ...
        FROM
            _timeudb_catalog.chunk normal_chunk,
            _timeudb_catalog.chunk compressed_chunk
        WHERE
...
col AS (
    ...
    WHERE
        ...
        AND compressed_column.atttypid != '_timeudb_internal.compressed_data'::regtype
...
DROP FUNCTION IF EXISTS timeudb_experimental.refresh_continuous_aggregate(REGCLASS, REGCLASS);
...
UPDATE _timeudb_catalog.continuous_agg SET finalized = FALSE;

ALTER TABLE _timeudb_catalog.continuous_agg
  ALTER COLUMN finalized SET NOT NULL,
  ALTER COLUMN finalized SET DEFAULT TRUE;

DROP PROCEDURE IF EXISTS timeudb_experimental.move_chunk(REGCLASS, NAME, NAME);
DROP PROCEDURE IF EXISTS timeudb_experimental.copy_chunk(REGCLASS, NAME, NAME);

-- we only create stub here to prevent dependency on a specific so
CREATE OR REPLACE FUNCTION timeudb_experimental.subscription_exec(
