...
ALTER TABLE _timeudb_catalog.compression_chunk_size DROP CONSTRAINT compression_chunk_size_pkey;
ALTER TABLE _timeudb_catalog.compression_chunk_size ADD CONSTRAINT compression_chunk_size_pkey PRIMARY KEY(chunk_id);

CREATE TABLE _timeudb_internal.job_errors (
...
CREATE VIEW timeudb_information.job_errors AS
SELECT
    job_id,
...
CREATE TABLE _timeudb_internal._tmp_bgw_job_stat AS SELECT * FROM _timeudb_internal.bgw_job_stat;
DROP TABLE _timeudb_internal.bgw_job_stat;
...
INSERT INTO _timeudb_internal.bgw_job_stat SELECT
...
FROM _timeudb_internal._tmp_bgw_job_stat;
DROP TABLE _timeudb_internal._tmp_bgw_job_stat;

GRANT SELECT ON TABLE _timeudb_internal.bgw_job_stat TO PUBLIC;

DROP FUNCTION IF EXISTS _timeudb_internal.hypertable_local_size(name, name);
DROP VIEW IF EXISTS _timeudb_internal.hypertable_chunk_local_size;

CREATE VIEW _timeudb_internal.hypertable_chunk_local_size AS
SELECT
...
FROM
    ...
    JOIN LATERAL _timeudb_internal.relation_size(
    ...
    LEFT JOIN LATERAL _timeudb_internal.relation_size(
...
CREATE FUNCTION _timeudb_internal.hypertable_local_size(
	schema_name_in name,
	table_name_in name)
RETURNS TABLE (
	table_bytes BIGINT,
...
$BODY$
    ...
    WITH _hypertable AS (
        SELECT
            id,
            _timeudb_internal.relation_size(format('%I.%I', schema_name, table_name)::regclass) AS relsize
        FROM
            _timeudb_catalog.hypertable
        WHERE
...
-- rebuild _timeudb_config.bgw_job
...
ALTER TABLE _timeudb_internal.bgw_job_stat DROP CONSTRAINT IF EXISTS bgw_job_stat_job_id_fkey;
ALTER TABLE _timeudb_internal.bgw_policy_chunk_stats DROP CONSTRAINT IF EXISTS bgw_policy_chunk_stats_job_id_fkey;

CREATE TABLE _timeudb_internal.tmp_bgw_job_seq_value AS SELECT last_value, is_called FROM _timeudb_config.bgw_job_id_seq;
...
SELECT setval('_timeudb_config.bgw_job_id_seq', last_value, is_called) FROM _timeudb_internal.tmp_bgw_job_seq_value;
DROP TABLE _timeudb_internal.tmp_bgw_job_seq_value;

-- new table as we want it
CREATE TABLE _timeudb_config.bgw_job (
  id INTEGER PRIMARY KEY DEFAULT nextval('_timeudb_config.bgw_job_id_seq'),
  ...
  hypertable_id  integer REFERENCES _timeudb_catalog.hypertable (id) ON DELETE CASCADE,
  config jsonb ,
  check_schema NAME,
  check_name NAME,
  timezone TEXT
...
FROM _timeudb_config.bgw_job_tmp ORDER BY id;

DROP TABLE _timeudb_config.bgw_job_tmp;
ALTER TABLE _timeudb_internal.bgw_job_stat ADD CONSTRAINT bgw_job_stat_job_id_fkey FOREIGN KEY(job_id) REFERENCES _timeudb_config.bgw_job(id) ON DELETE CASCADE;
ALTER TABLE _timeudb_internal.bgw_policy_chunk_stats ADD CONSTRAINT bgw_policy_chunk_stats_job_id_fkey FOREIGN KEY(job_id) REFERENCES _timeudb_config.bgw_job(id) ON DELETE CASCADE;
...
-- Recreate _timeudb_catalog.dimension table with the compress_interval_length column --
CREATE TABLE _timeudb_internal.dimension_tmp
AS SELECT * from _timeudb_catalog.dimension;

CREATE TABLE _timeudb_internal.tmp_dimension_seq_value AS
SELECT last_value, is_called FROM _timeudb_catalog.dimension_id_seq;

--drop foreign keys on dimension table
ALTER TABLE _timeudb_catalog.dimension_partition DROP CONSTRAINT
dimension_partition_dimension_id_fkey;
ALTER TABLE _timeudb_catalog.dimension_slice DROP CONSTRAINT
...
ALTER EXTENSION timeudb DROP TABLE _timeudb_catalog.dimension;
ALTER EXTENSION timeudb DROP SEQUENCE _timeudb_catalog.dimension_id_seq;
DROP TABLE _timeudb_catalog.dimension;
...
INSERT INTO _timeudb_catalog.dimension
...
SELECT id, hypertable_id, column_name, column_type,
  ...
  partitioning_func, interval_length,
  integer_now_func_schema, integer_now_func
FROM _timeudb_internal.dimension_tmp;

ALTER SEQUENCE _timeudb_catalog.dimension_id_seq OWNED BY _timeudb_catalog.dimension.id;
SELECT setval('_timeudb_catalog.dimension_id_seq', last_value, is_called) FROM _timeudb_internal.tmp_dimension_seq_value;
...
ALTER TABLE _timeudb_catalog.dimension_partition ADD CONSTRAINT
dimension_partition_dimension_id_fkey FOREIGN KEY (dimension_id)
REFERENCES _timeudb_catalog.dimension(id) ON DELETE CASCADE;
ALTER TABLE _timeudb_catalog.dimension_slice ADD CONSTRAINT
dimension_slice_dimension_id_fkey FOREIGN KEY (dimension_id)
REFERENCES _timeudb_catalog.dimension(id) ON DELETE CASCADE;

--cleanup
DROP TABLE _timeudb_internal.dimension_tmp;
DROP TABLE _timeudb_internal.tmp_dimension_seq_value;

GRANT SELECT ON _timeudb_catalog.dimension_id_seq TO PUBLIC;
GRANT SELECT ON _timeudb_catalog.dimension TO PUBLIC;

-- end recreate _timeudb_catalog.dimension table --
...
DROP FUNCTION IF EXISTS _timeudb_internal.cagg_migrate_plan_exists (INTEGER);
...
ALTER TABLE _timeudb_catalog.continuous_agg_migrate_plan
...
ALTER TABLE _timeudb_catalog.continuous_agg_migrate_plan
    ADD CONSTRAINT continuous_agg_migrate_plan_mat_hypertable_id_fkey
        ...
        REFERENCES _timeudb_catalog.continuous_agg (mat_hypertable_id);

ANALYZE _timeudb_catalog.continuous_agg;

-- only create stub here to not introduce shared library dependency
CREATE FUNCTION _timeudb_internal.drop_stale_chunks(
    node_name NAME,
    chunks integer[] = NULL
) RETURNS VOID
