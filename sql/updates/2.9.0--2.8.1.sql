...
ALTER TABLE _timeudb_catalog.compression_chunk_size ADD CONSTRAINT compression_chunk_size_pkey PRIMARY KEY(chunk_id,compressed_chunk_id);
...
DELETE FROM _timeudb_config.bgw_job WHERE id = 2;
...
INSERT INTO _timeudb_config.bgw_job(id, application_name, schedule_interval, max_runtime, max_retries, retry_period, proc_schema, proc_name, owner, scheduled, hypertable_id, config)
SELECT id, application_name, schedule_interval, max_runtime, max_retries, retry_period, proc_schema, proc_name, owner, scheduled, hypertable_id, config FROM _timeudb_config.bgw_job_tmp ORDER BY id;
...
ALTER TABLE _timeudb_internal.bgw_policy_chunk_stats
      ADD CONSTRAINT bgw_policy_chunk_stats_chunk_id_fkey
          FOREIGN KEY (chunk_id) REFERENCES _timeudb_catalog.chunk(id)
          ON DELETE CASCADE,
      ADD CONSTRAINT bgw_policy_chunk_stats_job_id_fkey
          FOREIGN KEY(job_id) REFERENCES _timeudb_config.bgw_job(id)
          ON DELETE CASCADE;

DROP FUNCTION _timeudb_internal.health;

-- Recreate _timeudb_catalog.dimension table without the compress_interval_length column --
...
DROP INDEX _timeudb_catalog.chunk_data_node_node_name_idx;
...
DROP FUNCTION _timeudb_internal.drop_stale_chunks;
