...
CREATE OR REPLACE FUNCTION _timeudb_functions.first_sfunc(internal, anyelement, "any")
RETURNS internal
AS '@MODULE_PATHNAME@', 'ts_first_sfunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.first_combinefunc(internal, internal)
RETURNS internal
AS '@MODULE_PATHNAME@', 'ts_first_combinefunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.last_sfunc(internal, anyelement, "any")
RETURNS internal
AS '@MODULE_PATHNAME@', 'ts_last_sfunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.last_combinefunc(internal, internal)
RETURNS internal
AS '@MODULE_PATHNAME@', 'ts_last_combinefunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.bookend_finalfunc(internal, anyelement, "any")
RETURNS anyelement
AS '@MODULE_PATHNAME@', 'ts_bookend_finalfunc'
LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.bookend_serializefunc(internal)
RETURNS bytea
AS '@MODULE_PATHNAME@', 'ts_bookend_serializefunc'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timeudb_functions.bookend_deserializefunc(bytea, internal)
RETURNS internal
...
CREATE OR REPLACE AGGREGATE @extschema@.first(anyelement, "any") (
    SFUNC = _timeudb_functions.first_sfunc,
    STYPE = internal,
    COMBINEFUNC = _timeudb_functions.first_combinefunc,
    SERIALFUNC = _timeudb_functions.bookend_serializefunc,
    DESERIALFUNC = _timeudb_functions.bookend_deserializefunc,
    PARALLEL = SAFE,
    FINALFUNC = _timeudb_functions.bookend_finalfunc,
    FINALFUNC_EXTRA
);
...
CREATE OR REPLACE AGGREGATE @extschema@.last(anyelement, "any") (
    SFUNC = _timeudb_functions.last_sfunc,
    STYPE = internal,
    COMBINEFUNC = _timeudb_functions.last_combinefunc,
    SERIALFUNC = _timeudb_functions.bookend_serializefunc,
    DESERIALFUNC = _timeudb_functions.bookend_deserializefunc,
    PARALLEL = SAFE,
    FINALFUNC = _timeudb_functions.bookend_finalfunc,
