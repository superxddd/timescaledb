...
CREATE OR REPLACE FUNCTION _timeudb_functions.calculate_chunk_interval(
...
CREATE OR REPLACE FUNCTION _timeudb_functions.chunk_status(REGCLASS) RETURNS INT
AS '@MODULE_PATHNAME@', 'ts_chunk_status' LANGUAGE C;

--given a chunk's relid, return the id. Error out if not a chunk relid.
CREATE OR REPLACE FUNCTION _timeudb_functions.chunk_id_from_relid(relid OID) RETURNS INTEGER
AS '@MODULE_PATHNAME@', 'ts_chunk_id_from_relid' LANGUAGE C STABLE STRICT PARALLEL SAFE;

-- Show the definition of a chunk.
CREATE OR REPLACE FUNCTION _timeudb_functions.show_chunk(chunk REGCLASS)
...
CREATE OR REPLACE FUNCTION _timeudb_functions.create_chunk(
       hypertable REGCLASS,
       slices JSONB,
...
CREATE OR REPLACE FUNCTION _timeudb_functions.create_chunk_table(
       hypertable REGCLASS,
       slices JSONB,
       schema_name NAME,
       table_name NAME)
...
CREATE OR REPLACE FUNCTION _timeudb_functions.freeze_chunk(
   chunk REGCLASS)
RETURNS BOOL AS '@MODULE_PATHNAME@', 'ts_chunk_freeze_chunk' LANGUAGE C VOLATILE;

CREATE OR REPLACE FUNCTION _timeudb_functions.unfreeze_chunk(
   chunk REGCLASS)
...
CREATE OR REPLACE FUNCTION _timeudb_functions.drop_chunk(
   chunk REGCLASS)
...
CREATE OR REPLACE FUNCTION _timeudb_functions.attach_osm_table_chunk(
   hypertable REGCLASS,
   chunk REGCLASS)
