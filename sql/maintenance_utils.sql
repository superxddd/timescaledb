...
CREATE OR REPLACE FUNCTION _timeudb_functions.create_compressed_chunk(
    chunk REGCLASS,
...
CREATE OR REPLACE FUNCTION _timeudb_functions.recompress_chunk_segmentwise(
    uncompressed_chunk REGCLASS,
    if_compressed BOOLEAN = true
...
CREATE OR REPLACE FUNCTION _timeudb_functions.get_compressed_chunk_index_for_recompression(
    uncompressed_chunk REGCLASS
...
CREATE OR REPLACE FUNCTION _timeudb_functions.makeaclitem(regrole, regrole, text, bool)
...
CREATE OR REPLACE PROCEDURE _timeudb_functions.repair_relation_acls()
LANGUAGE SQL AS $$
  WITH
    badrels AS (
	SELECT oid::regclass
    ...
    pickacls AS (
      SELECT oid::regclass,
	     _timeudb_functions.makeaclitem(
	         b.grantee,
		 b.grantor,
...
CREATE OR REPLACE FUNCTION _timeudb_functions.remove_dropped_chunk_metadata(_hypertable_id INTEGER)
